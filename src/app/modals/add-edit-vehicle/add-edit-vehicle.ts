...
export class AddEditVehicle implements OnInit {
  ...
  newModelMake: string;
  newModel: string;
  newBatteryCapacity: number;
  newEPARange: number;
  newOdometerStart: number;
  ...
  ngOnInit () {
    ...
    if (this.vehicleEdit) {
      this.newModelYear = this.vehicleEdit.modelYear;
      this.newModelMake = this.vehicleEdit.modelMake;
      this.newModel = this.vehicleEdit.model;
      this.newBatteryCapacity = this.vehicleEdit.batteryCapacity;
      this.newEPARange = this.vehicleEdit.EPARange;
      this.newOdometerStart = this.vehicleEdit.odometerStart;
  ...
  getFormTitle (): string {
    if (!this.vehicleEdit) {
      return this.vehicleCopy.formAddVehicleTitle;
    }
  ...
  getModelYears (): string[] {
    ...
    return (years.fill(2006)).map((base, index) => (base + index).toString())
        .reverse();
  }
